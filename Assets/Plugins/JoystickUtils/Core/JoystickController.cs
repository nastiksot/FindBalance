...
namespace Plugins.JoystickUtils.Core
{
    public class JoystickController : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
    {
        [Header("Settings")] [SerializeField, Range(1, 15)]
        private float Radio = 5; //the ratio of the circumference of the joystick
        ...
        [Header("Reference")] [SerializeField] private RectTransform StickRect; //The middle joystick UI
        ...
        /// <summary>
        /// Reset center rect position
        /// </summary>
        public void ResetJoystickPosition()
        {
            StickRect.position = DeathArea;
            enabled = false;
            StartCoroutine(StartWithDelay(0.01f, () =>
            {
                enabled = true;
                backImage.CrossFadeColor(NormalColor, 0.01f, true, true);
                stickImage.CrossFadeColor(NormalColor, 0.01f, true, true);
            }));
        }

        private IEnumerator StartWithDelay(float second, Action action)
        {
        ...
        public void OnDrag(PointerEventData data)
        {
            ...
            if (data.pointerId == lastId)
            {
                ...
                Vector3 position = global::JoystickUtils.TouchPosition(m_Canvas, GetTouchID);
        ...
        private float radio
        {
            get { return (Radio * 5 + Mathf.Abs((diff - CenterReference.position.magnitude))); }
        }

        private float smoothTime
        {
            get { return (1 - (SmoothTime)); }
        }
        ...
        {
            get { return (StickRect.position.x - DeathArea.x) / Radio; }
        }
        ...
        {
            get { return (StickRect.position.y - DeathArea.y) / Radio; }
        }
    }
}